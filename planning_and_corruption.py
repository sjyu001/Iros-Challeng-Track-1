...
class VLMInference:
    ...
    def process_sample(self, record: Dict[str, Any]) -> str:
        ...
        fewshots  = record.get("fewshot_examples", []) or []
        ...
        # few-shot 필터
        if self.k_shot > 0 and isinstance(fewshots, list):
            cat = category
            def ok(s):
                sq = (s.get("question","") or "").lower()
                good_cat = (("object" in cat and "object" in sq) or
                            ("scene" in cat and "scene" in sq) or
                            ("planning" in cat and "plan" in sq) or
                            ("prediction" in cat and "predict" in sq))
                good_cam = any(cam.lower() in sq for cam in self.CAMERA_ORDER)
                key = any(k in sq for k in ["distance","left","right","lane","brake","yield","turn","occluded","moving","stopped"])
                return good_cat or good_cam or key
            fewshots = [s for s in fewshots if ok(s)][: self.k_shot]
        else:
            fewshots = []
        ...
        if fewshots:
            messages.append({"role": "system", "content": f"Few-Shot Examples ({len(fewshots)}):"})
            for s in fewshots:
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--k", type=int, default=10,
                        help="Max number of few-shot examples from record.fewshot_examples")
